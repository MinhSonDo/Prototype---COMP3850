...
set2Data.Set2.map(record => {
    console.log(record)
    const newSet2 = new Set2({
        question: record.question,
        answers: record.answers,
        correct: record.correct
    })
    newSet2.save().then(result => {
        console.log("Set2 record saved")
    })
})

set3Data.Set3.map(record => {
    console.log(record)
    const newSet3 = new Set3({
        question: record.question,
        answers: record.answers,
        correct: record.correct
    })
    newSet3.save().then(result => {
        console.log("Set3 record saved")
    })
})